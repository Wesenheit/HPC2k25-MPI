...
void Node::get_graph_comm(MPI_Comm *com) {
  ...
  MPI_Dist_graph_create_adjacent(world, table.node.size(), table.node.data(),
                                 table.how_many.data(), table.node.size(),
                                 table.node.data(), table.how_many.data(), info,
                                 1, com);
  MPI_Info_free(&info);
}
...
void Node::finalize_rma_window() { MPI_Win_free(&tenative_win); }
...
void Node::relax(Vertex u, Vertex v, DVar d, int bucket_th) {
  if (v >= lower && v <= upper) {
    ...
    MPI_Raccumulate(&(que.mess_arr.back().distance), 1, MPI_UNSIGNED_LONG_LONG,
                    dest, target_disp, 1, MPI_UNSIGNED_LONG_LONG, MPI_MIN,
                    tenative_win, &que.req_arr.back());
