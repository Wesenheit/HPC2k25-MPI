...
Node::Node(int Delta, Path in, MPI_Comm com) {
  ...
  tenative =
      std::vector<DVar>(upper - lower + 1, std::numeric_limits<DVar>::max());
  que.mess_arr.reserve(MAX_QUE_SIZE);
  que.req_arr.reserve(MAX_QUE_SIZE);
  que.dest.reserve(MAX_QUE_SIZE);
