...
def compare_files(k, path):
    ...
    for x in range(k):
        ...
        try:
            with (
                open(os.path.join(path, file1), "r") as f1,
                open(os.path.join(path, file2), "r") as f2,
            ):
                lines1 = f1.readlines()
                lines2 = f2.readlines()
                ...
                if len(lines1) != len(lines2):
                    print(
                        f"Mismatch in number of lines: {len(lines1)} vs {len(lines2)}"
                    )
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", type=str, required=True, help="Path to directory containing the files."
    )
